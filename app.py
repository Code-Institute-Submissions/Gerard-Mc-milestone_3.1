...
def check():
    ...
    # Checks the GPU in the data base to see if the game
    # has been found to be compatible already from a previous search
    check_database = mongo.db.gpu.find_one(
        {'model': f"{user_gpu_name}"},
        {'games': {'$elemMatch': {'name': f"{user_game_name}"}}})
    if check_database:
    ...
    if gpu_requirements != "":
        ...
        return render_template("result.html", info_message=message_fail)
